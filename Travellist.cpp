...
int PassengerList::totalPassengersFromSource(std::string source){
int count=0;
std::list<Passenger>::iterator iter;
    for(iter=TrainList.begin();iter!=TrainList.end();++iter)
    {
    if (iter->getSource()==source)
    {
    count++;
    }
    }
...
int PassengerList::totalPassengersToADestination(std::string destination){
int count=0;
std::list<Passenger>::iterator iter;
    for(iter=TrainList.begin();iter!=TrainList.end();++iter)
    {
    if (iter->getDestination()==destination)
    {
    count++;
    }
    }
...
int PassengerList::totalPassengersFromSourcetoDestination(std::string source,std::string destination){
int count=0;
std::list<Passenger>::iterator iter;
    for(iter=TrainList.begin();iter!=TrainList.end();++iter)
    {
    if (iter->getDestination()==destination  && iter->getSource()==source)
    {
    count++;
    }
    }
